...
const BundleOutputDir = path.join(__dirname, '..', '..', 'EmployeeVacationCalendar.WebAPI/EmployeeVacationCalendar.WebAPI.App/wwwroot');
console.log("__dirname " + __dirname);
console.log("Path " + BundleOutputDir);
exports.BundleOutputDir = BundleOutputDir;
        ...
        plugins.push(new FileManagerPlugin({
            onStart: [{
                delete: [
                    'dist',
                    'lib',
                    path.join(BundleOutputDir, 'dist'),
                    path.join(BundleOutputDir, 'index.html')
                ]
            }],
            onEnd: [{
                copy: [
                    // Doesn't matter in which way you choose to do this. It will still get done
                    { source: "src/assets/*", destination: path.resolve(BundleOutputDir, "dist/assets") }
                ]
            }]
        }));
